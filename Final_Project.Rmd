...
# percentage change of unemployed people by immigrant status
a <- asec_allyears %>%
...
a %>%
  #group_by(year,immigrant)%>%
  ggplot(aes(x= year, y= pct_unemployed, group = immigrant, color = immigrant)) +
  geom_line() +
  geom_point(size = 1) +
   labs(
    title = "Unemployment Rate Rose in the US",
    caption = "Source: IPUMS Data",
    x = "",
    y = "Unemployment Rate",
  ) +
