...
We initially converted all categorical variables as dummy variables in our preprocessing, however, we realized that having over 1000 final predictors _with_ hyperparameter tuning was too taxing for R, and we decided to omit select categorical variables that had dozens of response options (e.g. industry and occupation). Finally, we normalized all numerical variables. 
...
While we understood the unlikelihood that our model trained on the entire US population would perform well on a specific subgroup, we wanted to test it out as data science curious novices. Would the model still fare better than random guessing (indicative when the ROC area under the curve is 0.5)? Below, we prepare our implementation data. 
...
Contrary to our expectations, our model could be generalized to a immigrant-only data set. Accuracy, specificity, and area under the curve metrics were even better than previous instances. Since our immigrant-only data set included immigrants of all status (naturalized citizens, Legal Permanent Residents, undocumented immigrants), it is likely that it was not distinguishable enough from the general US population. 
